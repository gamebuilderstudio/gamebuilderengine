package pxBitmapFont
{
	...
	public class PxBitmapFont 
	{
		...
		public function loadPixelizer(pBitmapData:BitmapData, pLetters:String):PxBitmapFont
		{
			...
			if (pBitmapData != null) 
			{
				...
				for (var letterID:int = 0; letterID < tileRects.length; letterID++)
				{
					...
					// store glyph
					setGlyph(_glyphString.charCodeAt(letterID), {image: bd, xadvance: currRect.width});
				}
			}
		...
		public function getPreparedGlyphs(pScale:Number, pColor:int, pUseColorTranform:Boolean = true):Array
		{
			...
			for (var i : * in _glyphs)
			{
				glyph = _glyphs[i].image;
				var bdWidth:int;
				var bdHeight:int;
				if (glyph != null)
				{
					...
					var newData : Object = {};
					for(var prop : * in _glyphs[i])
					{
						newData[prop] = _glyphs[i][prop];
					}
					newData.image = preparedGlyph;
					result[i] = newData;
				}
			}
		...
		public function getFontData():String 
		{
			...
			var x : int = 0;
			for (var i: * in _glyphString) 
			{
				var charCode:int = _glyphString.charCodeAt(i);
				var glyph:BitmapData = _glyphs[charCode].image;
				output += _glyphString.substr(x, 1);
				output += glyph.width;
				output += glyph.height;
		...
		/**
		 * Get a list of all available characters on the image font.
		 **/
		public function getAvailableChars():Array
		{
			...
			{
				list.push(_glyphString.substr(i,1));
			}
			return list;
		}
		
		private function setGlyph(pCharID:int, glyData : Object):void 
		{
			var pBitmapData:BitmapData = glyData.image;
			if (_glyphs[pCharID] != null)
			{
				_glyphs[pCharID].image.dispose();
			}
			
			_glyphs[pCharID] = glyData;
