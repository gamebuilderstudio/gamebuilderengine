package com.pblabs.engine.util {
	...
	public class MathUtil {
		...
		public static function getRandomColor(minLum : uint = 0, maxLum : uint = 0xFF, b32 : Boolean = false, randAlpha : Boolean = false) : uint {
			...
			var r : uint = MathUtil.randomInt(minLum, maxLum);
			var g : uint = MathUtil.randomInt(minLum, maxLum);
			var b : uint = MathUtil.randomInt(minLum, maxLum);
			
			if (!b32)
				return r << 16 | g << 8 | b;
			else {
				var a : uint = randAlpha ? MathUtil.randomInt(0, 255) : 255;
